        ...
        function Tree(container, options) {
            console.log(options)
                ...
                var defaultOptions = {
                    ...
                    values: [],
                    disables: [],
                    beforeLoad: null,//Sau khi gọi qua ajax cần điều chỉnh lại dữ liệu
                    editCallback: null,
                    approvedCallback: null,
                    peddingCallback: null,
                    addsubCallback: null,
                    deleteCallback: null,
                    loaded: null,
                    url: null,
            ...
            Tree.prototype.DeleteItemClick = function (target) {
                if (typeof this.options.deleteCallback === 'function') {
                    this.options.deleteCallback(target);
                } else {
                    alert("Chưa cấu hình callback cho sự kiện xóa");
                }
            };
            Tree.prototype.EditItemClick = function (target) {
                if (typeof this.options.editCallback === 'function') {
                    this.options.editCallback(target);
                } else {
                    alert("Chưa cấu hình callback cho sự kiện sửa");
                }
            };
            Tree.prototype.ApprovedItemClick = function (target) {
                if (typeof this.options.approvedCallback === 'function') {
                    this.options.approvedCallback(target);
                } else {
                    alert("Chưa cấu hình callback cho sự kiện duyệt");
                }
            };
            Tree.prototype.PeddingItemClick = function (target) {
                if (typeof this.options.peddingCallback === 'function') {
                    this.options.peddingCallback(target);
                } else {
                    alert("Chưa cấu hình callback cho sự kiện hủy duyệt");
                }
            };
            Tree.prototype.AddSubItemClick = function (target) {
                if (typeof this.options.addsubCallback === 'function') {
                    this.options.addsubCallback(target);
                } else {
                    alert("Chưa cấu hình callback cho sự kiện tạo menu con");
                }
            };
                    ...
                    if (xhr.readyState === XMLHttpRequest.DONE) {
                        if (xhr.status === 200) {
                            ...
                            try {
                                ...
                                data = xhr.responseText; // Nếu không thể parse JSON, giữ nguyên response text
                            }

                            if (beforeLoad) {
                                data = beforeLoad(data);
                            }

                            callback(data);
                            console.timeEnd('load');
                        } else {
                            console.error('Register Tree Failed: to fetch data from API');
                        }
                    }
                };
                xhr.send();
            ...
            Tree.prototype.buildTree = function (nodes, depth) {
                ...
                if (nodes && nodes.length) {
                    ...
                    nodes.forEach(function (node) {
                        var liEle = Tree.createLiEle(_this2, node, depth === _this2.options.closeDepth - 1);
            ...
            Tree.prototype.bindEvent = function (ele) {
                ...
                ele.addEventListener('click', function (e) {
                    var target = e.target;
                    var allHighline = document.querySelector(_this3.container).querySelectorAll('.highline');
                    allHighline.forEach(function (quickTool) {
                        quickTool.classList.remove('highline');
                    });
                    target.classList.add('highline')
                ...
                ele.addEventListener('contextmenu', function (e) {
                    ...
                    var allQuickTool = document.querySelector(_this3.container).querySelectorAll('.quickTool');
                    allQuickTool.forEach(function (quickTool) {
                        quickTool.classList.add('d-none');
                    });
                    var target = e.target;
                    var parentLi = findParentLi(target);
                    if (parentLi) {
                        var quickTool = parentLi.querySelector('.quickTool');
                        if (quickTool) {
                            quickTool.classList.remove('d-none');
                        }


                        var buttonEdit = quickTool.querySelector(".edit");
                        var buttonAddSubItem = quickTool.querySelector(".addsubitem");
                        var buttondelete = quickTool.querySelector(".delete");
                        var buttonapproved = quickTool.querySelector(".approved");
                        var buttonpendding = quickTool.querySelector(".pendding");



                        if (buttonEdit) {
                            $(buttonEdit).off("click").on("click", function () {
                                _this3.EditItemClick(buttonEdit.getAttribute("data-nodeid"));
                            });
                        }

                        if (buttonAddSubItem) {
                            $(buttonAddSubItem).off("click").on("click", function () {
                                _this3.AddSubItemClick(buttonAddSubItem.getAttribute("data-nodeid"));
                            });
                        }

                        if (buttonapproved) {
                            $(buttonapproved).off("click").on("click", function () {
                                _this3.ApprovedItemClick(buttonapproved.getAttribute("data-nodeid"));
                            });
                        }


                        if (buttonpendding) {
                            $(buttonpendding).off("click").on("click", function () {
                                _this3.PeddingItemClick(buttonpendding.getAttribute("data-nodeid"));
                            });
                        }


                        if (buttondelete) {
                            $(buttondelete).off("click").on("click", function () {
                                _this3.DeleteItemClick(buttonEdit.getAttribute("data-nodeid"));
                            });
                        }
            ...
            //Đăng ký sự kiện nhấp chuột phải
            Tree.prototype.onRightClick = function (id) {
                console.time('onRightClick');
            ...
            Tree.createLiEle = function (tree, node, closed) {
                ...
                if (_this4.options.selectMode && _this4.options.selectMode === "checkbox") {
                ...
                if (node.ModerationStatus && node.ModerationStatus != 0) label.classList.add('pending');
                ...
                // Thêm phần tử quickTool vào li
                var quickTool = document.createElement('span');
                quickTool.classList.add('quickTool');
                quickTool.classList.add('d-none');


                var editLink = document.createElement('a');
                editLink.setAttribute('data-nodeid', node.id);
                editLink.classList.add('edit');
                var editImg = document.createElement('img');
                editImg.setAttribute('border', '0');
                editImg.setAttribute('title', 'Sửa menu');
                editImg.setAttribute('src', '/202404/images/act_edit.png');
                editLink.appendChild(editImg);
                quickTool.appendChild(editLink);



                var addSubLink = document.createElement('a');
                addSubLink.setAttribute('data-nodeid', node.id);
                addSubLink.classList.add('addsubitem');
                var addSubLinkImg = document.createElement('img');
                addSubLinkImg.setAttribute('border', '0');
                addSubLinkImg.setAttribute('title', 'Tạo sub menu');
                addSubLinkImg.setAttribute('src', '/202404/images/addsubitem.png');
                addSubLink.appendChild(addSubLinkImg);
                quickTool.appendChild(addSubLink);



                var hideLink = document.createElement('a');
                hideLink.setAttribute('data-nodeid', node.id);
                hideLink.classList.add('hide');
                var hideImg = document.createElement('img');
                hideImg.setAttribute('border', '0');
                if (node.ModerationStatus && node.ModerationStatus != 0) {
                    hideLink.classList.add('approved');
                    hideImg.setAttribute('title', 'Duyệt');
                    hideImg.setAttribute('src', '/202404/images/act_show.png');
                }
                else {
                    hideLink.classList.add('pendding');
                    hideImg.setAttribute('title', 'Hủy duyệt');
                    hideImg.setAttribute('src', '/202404/images/act_hide.png');
                }

                hideLink.appendChild(hideImg);
                quickTool.appendChild(hideLink);



                var deleteLink = document.createElement('a');
                deleteLink.setAttribute('data-nodeid', node.id);
                deleteLink.classList.add('delete');
                var deleteImg = document.createElement('img');
                deleteImg.setAttribute('border', '0');
                deleteImg.setAttribute('title', 'Xóa menu');
                deleteImg.setAttribute('src', '/202404/images/act_delete.gif');
                deleteLink.appendChild(deleteImg);



                quickTool.appendChild(deleteLink);

                li.appendChild(quickTool);
                return li;
            };
